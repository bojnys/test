...
@section('content')
    <span style="font-style: italic;">{{$user->city}}</span>@if(Auth::check())
        @if($user->slug == auth()->user()->slug || ($user->role == 1 && auth()->user()->role == 5))
            <a href="/user/{{$user->slug}}/edit" class="btn btn-primary" role="button" style="padding-left: 5px; padding-right: 5px; padding-top: 3px; padding-bottom: 3px;">Edit</a>
        @endif
    @endif<br>(
    @if($user->role == 5)
    ...
    @if($user->posts()->count() > 0)
        ...
        @endforeach
        <br>
        @if($user->posts()->count() > 3)<br>
            <a href="/user/{{$user->slug}}/posts" class="btn btn-info" role="button">Laad meer posts.</a>
        @endif
        <hr>
    @endif
